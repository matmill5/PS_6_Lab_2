...
OdomTf::OdomTf(ros::NodeHandle* nodehandle) : nh_(*nodehandle) { // constructor
    ...
    stfOdomWrtMap_ = stfAmclBaseLinkWrtMap_;
    stfOdomWrtMap_.frame_id_ = "map"; // declare the respective frames
    stfOdomWrtMap_.child_frame_id_ = "odom"; 

    odom_subscriber_ = nh_.subscribe("/odom", 1, &OdomTf::odomCallback, this); //subscribe to odom messages

    //wait for odom publication:
    ...
    ROS_INFO("stfBaseLinkWrtOdom_");
    xform_utils.printStampedTf(stfBaseLinkWrtOdom_);
...
void OdomTf::initializeSubscribers() {
    ...
    odom_subscriber_ = nh_.subscribe("/odom", 1, &OdomTf::odomCallback, this); //subscribe to odom messages
    amcl_subscriber_ = nh_.subscribe("/amcl_pose", 1, &OdomTf::amclCallback, this); //subscribe to amcl_pose

    // add more subscribers here, as needed
}

//callback fnc based on odom
void OdomTf::odomCallback(const nav_msgs::Odometry& odom_rcvd) {
    ...
    //stfOdomWrtBase_.child_frame_id_ = "odom"; // make this legal for multiply
    ...
    //We can't publish base_frame w/rt odom frame, because base_frame is already a child of the "odom" frame
    // published by our simulation  (this will not be a problem with a real robot)
    //here's a trick: publish the odom frame as a child of the robot's base_frame to avoid a kinematic loop
    //rviz will then be able to transform the odom frame appropriately in whatever fixed frame is chosen for display
    // use stamped_transform_inverse() function to invert base w/rt odom into odom w/rt base
    stfOdomWrtBase_ = xform_utils.stamped_transform_inverse(stfBaseLinkWrtOdom_); 
    //use the TRANSFORM PUBLISHER to send out this transform on the tf topic
    br_.sendTransform(stfOdomWrtBase_);
    
    //OdomWrtMap is transform of our odom frame with respect to the map frame.
    ...
    //cout<<endl<<"stfOdomWrtMap_"<<endl;
    //xform_utils.printStampedTf(stfOdomWrtMap_);     
    // cout<<endl<<"stfBaseLinkWrtOdom_"<<endl;
    //xform_utils.printStampedTf(stfBaseLinkWrtOdom_);       
    
    //cascde the frames: for frames b==base_link, m==map, do==odom,
    // 
    // T_b/m = m^T_b = m^T_do * do^T_b = T_do/m * T_b/do
    // put the answer in stfEstBaseWrtMap_
    xform_utils.multiply_stamped_tfs(stfOdomWrtMap_,stfBaseLinkWrtOdom_,stfEstBaseWrtMap_);
    ...
    // the map frame, using amcl and using the imperfect () odom estimate
...
//  at each such update, get the most current base-frame w/rt  odom
//  figure out from this where is the  odom frame w/rt the map
// we can assume that the -odom frame moves only slowly with respect to the map, so infrequent updates should be OK
// We can re-use the transform: odom w/rt map repeatedly (e.g. at 50Hz, coincident with -odom publication updates)
//  continue to use the same odom w/rt map transform until we get a new one  (which should be pretty close to the previous one)
void OdomTf::amclCallback(const geometry_msgs::PoseWithCovarianceStamped& amcl_rcvd) {
    ...
    stfOdomWrtBase_ = xform_utils.stamped_transform_inverse(stfBaseLinkWrtOdom_); 
    //cout<<endl<<"stfOdomWrtBase_"<<endl;
    //xform_utils.printStampedTf(stfOdomWrtBase_);    
    
    if (!xform_utils.multiply_stamped_tfs(stfAmclBaseLinkWrtMap_,stfOdomWrtBase_,stfOdomWrtMap_)) {
        ROS_WARN("stfAmclBaseLinkWrtMap_,stfOdomWrtBase_ multiply invalid" );
    }
   ...
   //publish the estimated odom frame w/rt the map.  This is useful for visualization, e.g. to see
    ...
    // existing name "odom", to avoid a kinematic-loop error in tf
    // result will be correctedodom w/rt map
    ...
    stfOdomWrtMap_.child_frame_id_ = "corrected_odom";
    //cout<<endl<<"stfOdomWrtMap_"<<endl;
    //xform_utils.printStampedTf(stfOdomWrtMap_);      
    br_.sendTransform(stfOdomWrtMap_);   
    //done publishing, so restore correct frame name
    stfOdomWrtMap_.child_frame_id_ = "dom"; //change this frame name back, for use
    // in odom callback
