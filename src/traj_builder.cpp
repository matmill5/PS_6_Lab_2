...
double TrajBuilder::convertPlanarQuat2Psi(geometry_msgs::Quaternion quaternion)
{
    ...
    double siny_cosp = 2 * (quat_w*quat_z);
    double cosy_cosp = 1 - 2 * (quat_z*quat_z);
                                  ...
                                  std::vector<nav_msgs::Odometry> &vec_of_states)
{
    ...
    ROS_INFO("psi_end: %f",psi_end);
    ROS_INFO("psi_start: %f",psi_start);
    ...
    { //delta-angle is too short for trapezoid
                                    ...
                                    std::vector<nav_msgs::Odometry> &vec_of_states)
{
    ...
    { //length is too short for trapezoid
